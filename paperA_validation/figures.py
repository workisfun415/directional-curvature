...
def fig7_aliasing(results: Path, out: Path):
    r = _load(results, "aliasing.json")["paths"]
    th = [0.1, 0.05, 0.025, 0.0125]
    fig, axes = plt.subplots(1, 3, figsize=(10, 3.2), sharey=False)
    for ax, path in zip(axes, ["f333->mixed", "f333->tangential", "f133->tangential"]):
        for dname, rows in r.items():
            ls = "o-" if dname.startswith("non") else "s--"
            ax.plot(th, rows[path]["bias_over_h"], ls, ms=4, label=dname)
        ax.set_xscale("log"); ax.set_yscale("log"); ax.set_xlabel(r"$\theta$"); ax.set_title(path, fontsize=9)
    axes[0].set_ylabel(r"sector bias / $h$"); axes[2].legend(fontsize=6)
    fig.tight_layout(); fig.savefig(out / "fig7_aliasing.pdf"); plt.close(fig)


def make_all(results: Path, out: Path):
    out.mkdir(parents=True, exist_ok=True)
    fig7_aliasing(results, out)
