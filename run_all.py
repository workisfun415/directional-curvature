...
def main():
    ...
    # ---------------- Section 7: radial aliasing (quadratic regression) + Section 11 comparison
    ali = cfg["aliasing"]
    rng = C.seed_for(seed, "aliasing")
    V = AL.planar_disk(ali["rays"], rng)
    designs = {"non-product (random radii per ray)":
               RA.RegressionDesign(V, [tuple(x) for x in rng.uniform(*ali["random_radius_range"], size=(ali["rays"], 2))])}
    for rs in ali["product_radii"]:
        designs[f"product {tuple(rs)}"] = RA.RegressionDesign(V, [tuple(rs)] * ali["rays"])
    alias = {"paths": AL.alias_paths(designs, h=ali["h"]),
             "A_vs_B_max_abs": {k: AL.ab_agreement(d, AL.unit_tensor((2, 2, 2)), 0.02, ali["h"]) for k, d in designs.items()},
             "exact_witness_gamma02_nonproduct": [str(x) for x in SA.gamma02_exact()],
             "exact_witness_gamma02_product": [str(x) for x in SA.gamma02_exact(radii=SA.WITNESS_RADII_PRODUCT)]}
    dump("aliasing.json", alias)
    dump("stencil_vs_regression.json", AL.stencil_vs_regression(
        V, (1.0, 2.0), ali["h"], mc["sigma"], ali["compare_theta"], C.seed_for(seed, "stencil_vs_reg"),
        n_mc=1000 if args.quick else 4000))
